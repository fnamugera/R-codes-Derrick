...
covdata<- readxl::read_xlsx("D:/Learnings/work/OJ/COVID19 Impact study/Study_on_the_Impact_of_COVID-19_on_the_Tourism_Industry_in_Uganda_in_the_last_one_year_March_2020-March_2021_-_all_versions_-_labels_-_2021-04-20-06-14-09.xlsx", sheet = 'Study on the Impact of COVID...')
...
```{r, echo=FALSE, message=FALSE, warning=FALSE,error=FALSE, comment=NA}
covdata %>% select(`6. Which services are offered by the company? (tick the relevant boxes)/Transport Services` :`6. Which services are offered by the company? (tick the relevant boxes)/Other (Specify?)`) %>% melt() %>% mutate(Service =case_when(  str_detect(variable,"Transport Services") ~ "Transport Services", str_detect(variable, "Booking/Ticketing services")~"Booking/Ticketing services", str_detect(variable , "Tour Guiding services")~"Tour Guiding services", str_detect(variable, "Food and Accommodation")~"Food and Accommodation", str_detect(variable, "Recreational services")~"Recreational services", str_detect(variable, "Other")~"Other",)) %>% group_by(Service) %>% summarise(Count = sum(value)) %>% mutate(Per = paste0(round(Count/sum(Count)*100), '%')) %>% adorn_totals(where = 'row') %>% kable()
