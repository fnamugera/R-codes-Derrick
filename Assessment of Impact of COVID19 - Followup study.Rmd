...
```{r , echo=FALSE, message=FALSE, warning=FALSE,error=FALSE, comment=NA}
covdata %>% group_by(`15. Have you had a decline or increase in your sales in the last 12 months compared to before?`) %>% drop_na(`15. Have you had a decline or increase in your sales in the last 12 months compared to before?`) %>% summarise(count=n()) %>% mutate(`percent(%)`=paste0(round(count/sum(count)*100),"%")) %>% kable()
```
